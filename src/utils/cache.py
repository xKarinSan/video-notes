...
class Cache:
    @time_counter
    def __init__(self):
        self._base_url = "./user_data"
        self._cache_path = self._base_url + "/cache"
        self._transcripts_path = self._base_url + "/transcripts_text"
        self._metadata_path = self._base_url + "/metadata"
        self._summaries_path = self._base_url + "/summaries"
        self.json_transcript_path = self._base_url + "/transcripts_json"
    ...
    @time_counter
    def save_video(
        self, video_id: str, video_info: VideoInfo, json_transcript: dict[any]
    ) -> None:
        url = video_info.url
        with dbm.open(self._cache_path, "c") as db:
            db[url] = video_id
        ...
        os.makedirs(self.json_transcript_path, exist_ok=True)
        with open(
            f"{self.json_transcript_path}/{video_id}.json", "w", encoding="utf-8"
        ) as f:
